#include "OpenGLParticleShader.h"
...
OpenGLParticleShader::OpenGLParticleShader(const std::string& programName) :
	m_ProgramId(0),
	m_VertexShaderId(0),
...
OpenGLParticleShader::~OpenGLParticleShader()
{
	DeleteShaders();
...
GLint OpenGLParticleShader::FindUniformLocation(const std::string& uniformName)
{
...
bool OpenGLParticleShader::CheckShaderCompileErrors(unsigned int shader, ShaderType type)
{
	int success;
...
void OpenGLParticleShader::DeleteShaders()
{
	if (m_VertexShaderId)
	{
...
bool OpenGLParticleShader::CompileShader(ShaderType type, const std::filesystem::path& filepath)
{
	if (!m_ProgramId)
	{
		m_ProgramId = glCreateProgram();
...
bool OpenGLParticleShader::AddTransformFeedback(const std::vector<std::string>& variableNames, TransformFeedbackType feedbacktype)
{
	...
	switch (feedbacktype)
	{
	case OpenGLParticleShader::TransformFeedbackType::INTERLEAVED:
	{
		bufferMode = GL_INTERLEAVED_ATTRIBS;
	} break;

	case OpenGLParticleShader::TransformFeedbackType::SEPARATE:
	{
...
bool OpenGLParticleShader::LinkProgram()
{
...
GLuint OpenGLParticleShader::ShaderId(ShaderType type)
{
...
void OpenGLParticleShader::Use()
{
	glUseProgram(m_ProgramId);
}

void OpenGLParticleShader::SetInt(const std::string& paramName, int value)
{
	glUniform1i(FindUniformLocation(paramName), value);
}

void OpenGLParticleShader::SetUint(const std::string& paramName, unsigned int value)
{
	glUniform1ui(FindUniformLocation(paramName), value);
}

void OpenGLParticleShader::SetFloat(const std::string& paramName, float value)
{
	glUniform1f(FindUniformLocation(paramName), value);
}

void OpenGLParticleShader::SetVector3f(const std::string& paramName, const glm::vec3& value)
{
	glUniform3f(FindUniformLocation(paramName), value.x, value.y, value.z);
}

void OpenGLParticleShader::SetMatrix3fv(const std::string& paramName, const glm::mat3& value)
{
	glUniformMatrix3fv(FindUniformLocation(paramName), 1, GL_FALSE, glm::value_ptr(value));
}

void OpenGLParticleShader::SetMatrix4f(const std::string& paramName, const glm::mat4& value)
{
